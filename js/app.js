...
//this object stores vital information regarding the game state, score and difficulty.
var gameState = {
    difficulty: 1,
    currentScore: 0,
    highScore : 0,
...
Enemy.prototype.update = function(dt) {
    //reads speed of this enemy instance, adds it to the x position, then multiplies it by dt
    //checks to see if entity has crossed the boundary. if so, sets its position back
    ...
    //to jitter around. so, if dt is too large a number we set it back to something more reasonable.
        dt = 0.1;
    }
...
var Player = function() {
    ...
    this.x = this.startx;
    this.y = this.starty;
    //newy and newx are used to make temporary changes to player position, in order to leave rendering
    //of final position to the player.render method.
    this.newx = this.x;
    this.newy = this.y;
...
Player.prototype.update = function() {
    ...
    //onto the canvas. Using newx and current x to diff between what has changed in
...
Player.prototype.collisionCheck = function() {
    //simply converts numbers to whole numbers, checks to see if player entity
    //is within 40px of enemy entity in terms of x,y, triggers playerLose state if true.
    for (var i = 0; i < allEnemies.length; i++) {
        if(Math.abs(player.x - allEnemies[i].x) < 50 && Math.abs(player.y - allEnemies[i].y) < 50) {
            gameState.currentState = "playerLose";
    //if the player has not collided with an enemy, we check to see if they have reached the water.
        } else if (this.y < 10) {
...
Player.prototype.positionReset = function() {
    //simply resets location of the player.
...
//Instantiates enemies and the player entity.
